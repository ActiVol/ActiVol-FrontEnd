      ...
      <div class="px-6 py-8">
        ...
        <form @submit.prevent="submitForm" class="space-y-8">
              ...
              <div>
                ...
                <div class="relative">
                  <input
                    ...
                    placeholder="输入活动名称"
                    required
                    :disabled="isEditing && !editingFields.title"
                  >
                  <button
                    v-if="isEditing"
                    type="button"
              ...
              <div>
                ...
                <div class="relative">
                  <input
                    ...
                    placeholder="输入活动地点"
                    required
                    :disabled="isEditing && !editingFields.location"
                  >
                  <button
                    v-if="isEditing"
                    type="button"
          ...
          <div class="bg-gray-50 p-6 rounded-lg">
            ...
            <div v-for="(date, index) in form.dates" :key="index" class="mb-4 p-4 bg-white rounded-md shadow-sm">
          ...
          <div class="bg-gray-50 p-6 rounded-lg">
            ...
            <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
              <div class="space-y-1 text-center">
                ...
                <div class="flex text-sm text-gray-600">
                  ...
                  >
                    <span>上传海报</span>
                    <input id="poster-upload" name="poster-upload" type="file" class="sr-only" @change="handlePosterUpload">
                  </label>
            ...
            <img v-if="form.posterUrl" :src="form.posterUrl" alt="Activity Poster" class="mt-4 max-w-xs rounded-lg shadow-md">
          </div>
              ...
              <div>
                ...
                <div class="relative">
                  <input
                    ...
                    class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                    required
                    :disabled="isEditing && !editingFields.organizer"
                  >
                  <button
                    v-if="isEditing"
                    type="button"
                    @click="toggleEdit('organizer')"
                    class="absolute inset-y-0 right-0 px-3 flex items-center text-sm font-medium text-blue-600 hover:text-blue-500"
                  >
              ...
              <div>
                ...
                <div class="relative">
                  <input
                    ...
                    class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                    required
                    :disabled="isEditing && !editingFields.organizerEmail"
                  >
                  <button
                    v-if="isEditing"
                    type="button"
                    @click="toggleEdit('organizerEmail')"
                    class="absolute inset-y-0 right-0 px-3 flex items-center text-sm font-medium text-blue-600 hover:text-blue-500"
                  >
...
const { fetchActivity, createActivity, updateActivity } = useActivities()
...
interface ActivityForm {
  title: string
  dates: { date: string; duration: number; participants: number; unlimited: boolean }[]
  location: string
  categories: string[]
  posterUrl: string
  organizer: string
...
const form = ref<ActivityForm>({
  title: '',
  dates: [{ date: '', duration: 0, participants: 0, unlimited: false }],
  location: '',
  categories: [],
  posterUrl: '',
  organizer: '',
...
const addDate = () => {
  form.value.dates.push({ date: '', duration: 0, participants: 0, unlimited: false })
}
...
const handleUnlimitedChange = (index: number) => {
  if (form.value.dates[index].unlimited) {
    form.value.dates[index].participants = 999
  }
}
...
const submitForm = async () => {
  try {
    if (isEditing.value) {
      await updateActivity(Number(route.params.id), form.value)
    } else {
